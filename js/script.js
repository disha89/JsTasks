...
if (marina) {
    ...
    const maxAge = 18 - age;
    console.log(`you can watch the video through ${maxAge} years`);
}

const berthYear = 2000;
const nowYear = 2023;
const ages = nowYear - berthYear;
...
console.log(`I am adult? ${answer}`);

const massa = prompt('How much do you weigh?');
const growth = prompt('How tall are you?');

let resalt = massa / Math.pow(growth, 2);

if (resalt <= 24.9 && resalt !== null && resalt !== undefined) {
    alert(`Everything is fine!${resalt}`);
} else if (resalt >= 24.9 && resalt !== null && resalt !== undefined) {  
    alert(`You Should Lose Weight!${resalt}`);
} else {
